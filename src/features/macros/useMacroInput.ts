...
import { M8_GRAPH, type Direction } from './m8Graph'
...
export const useMacroInput = (connection?: ConnectedBus) => {
    ...
    useEffect(() => {
        const textHandler = (data: CharacterCommand) => {
            if ((data.pos.y === PAGENAME_ROW_INDEX) ||
                (data.pos.y >= MINIMAP_ROW_MIN && data.pos.x >= MINIMAP_COL_MIN)
            ) {
                const x = data.pos.x
                const y = data.pos.y
                ...
                stability.current.timer = setTimeout(() => {
                    if (stability.current.version === v) {
                        ...
                        const cleaned = raw
                            .replace(/\s([0-9a-fA-F]+|[0-9a-fA-F]{2})\b/g, '')
                            .replace(/[ *]/g, '')
    ...
    function handleInput(ev: KeyboardEvent, _isDown: boolean) {
    ...
    function planAndSendPath(targetPage: string) {
